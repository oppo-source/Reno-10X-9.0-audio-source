...
class URICertStore extends CertStoreSpi {
    ...
    public synchronized Collection<X509CRL> engineGetCRLs(CRLSelector selector)
        ...
        try {
            ...
            //#ifdef VENDOR_EDIT
            //Ming.Liu@PSW.CN.WiFi.Network.internet.1302417, 2018/03/09,
            //add for: [set read timeout so the thread can exit]
            /// M: Setup read time @{
            String readTimeoutVal = java.security.Security.getProperty("crl.read.timeout");
            System.out.println("crl.read.timeout:" + readTimeoutVal);
            if (readTimeoutVal != null) {
                int readTimeout = 0;
                try {
                    readTimeout = Integer.parseInt(readTimeoutVal);
                } catch (Exception e) {
                    System.out.println("read time parse error:" + e);
                }
                System.out.println("Configure readTimeout:" + readTimeout + " seconds");
                if (readTimeout != 0) {
