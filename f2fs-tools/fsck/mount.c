...
#ifdef VENDOR_EDIT
//Fangfang.Hui@PSW.AD.Storage.FileSystem.1593159, 2018/10/18, Add for change from compare the whole string to compare kernel version
#define IS_DIGIT_CHAR(c) ((c) >= '0' && (c) <= '9')

int compare_kernel_version_number(__u8 *oldInfo, __u8 *curInfo)
{
	char oldVersion[VERSION_LEN + 1], curVersion[VERSION_LEN + 1];
	int getOldVersion = 0, getNewVersion = 0;
	int i, j;
	//get old record kernel version number
	for (i = 0; i < VERSION_LEN; i++) {
		if (IS_DIGIT_CHAR(oldInfo[i]))
			break;
	}
	for (j = 0; i < VERSION_LEN; i++, j++) {
		if (oldInfo[i] == '-' || oldInfo[i] == '+') {
			oldVersion[j] = '\0';
			getOldVersion = 1;
			break;
		} else {
			oldVersion[j] = oldInfo[i];
		}
	}

	//get current kernel version number
	for (i = 0; i < VERSION_LEN; i++) {
		if (IS_DIGIT_CHAR(curInfo[i]))
			break;
	}
	for (j = 0; i < VERSION_LEN; i++, j++) {
		if (curInfo[i] == '-' || curInfo[i] == '+') {
			curVersion[j] = '\0';
			getNewVersion = 1;
			break;
		} else {
			curVersion[j] = curInfo[i];
        }
	}
	oldVersion[VERSION_LEN] = '\0';
	curVersion[VERSION_LEN] = '\0';

	//MSG(0, "original kernel version: %s\n", oldVersion);
	//MSG(0, "current kernel version: %s\n", curVersion);

	if (getOldVersion && getNewVersion)
		return strcmp(oldVersion, curVersion);
	else
		return memcmp(oldInfo, curInfo, VERSION_LEN);
}
#endif /* VENDOR_EDIT */
...
#ifndef VENDOR_EDIT
//Fangfang.Hui@PSW.AD.Storage.FileSystem.1593159, 2018/10/18, Modify for change from compare the whole string to compare kernel version
			c.auto_fix = 0;
			c.fix_on = 1;
#else
			if (compare_kernel_version_number(c.sb_version, c.version)) {
				c.auto_fix = 0;
				c.fix_on = 1;
			} else {
				MSG(0, "Info: kernel version number is same, ignore fix_on\n");
			}
#endif /* VENDOR_EDIT */
		}
