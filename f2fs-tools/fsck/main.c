...
static int do_resize(struct f2fs_sb_info *sbi)
{
	...
	if (!c.target_sectors)
#if defined(_CORETECH_ANDROID_RESIZE_)
        /* For userdata partition. There is a 16 KB encryption stored at the
            end of the partition. We needs to reserve the space for this key.
            So reduce the target sector number by 4 sectors. (4*4096 Bytes) */
        //config.target_sectors = config.total_sectors - 4;
        //wwei add start
        //ufs
        c.target_sectors = c.total_sectors  -  4;
        //emmc
        //c.target_sectors = c.total_sectors  -  (16 * 1024) / 512;
        MSG(0, "Info: Total sectors = %d, Target sectors = %d", c.total_sectors, c.target_sectors);
        //wwei add end
#else
